...
This document reflects state as of 2026-04-17 after six major passes: the 2026-04-12 **sweep fold-in + data integrity pass**, the 2026-04-16 **realism schema redesign** + V1 **data-upkeep run** (266 projects populated across 92 operators, one ghost deletion), the 2026-04-17 **data-upkeep V2/V3/V4 scaffold + affordability field** (affordability as a profile-relative counterpart to cost_tier, end-to-end schema → UI; data-upkeep skill extended to cover enrichment, area research, and grade recalibration under flag-driven scope), the 2026-04-17 **V2/V4 research wave** (15 parallel agents, 5 V2 operator enrichment batches + 10 V4 area research batches, ~5,700 field-level proposals written to `context/data-upkeep/proposals/` — 7 critical corrections applied to source, full enrichment apply staged for a follow-up session; run report at `context/data-upkeep/runs/2026-04-17-full.md`), and the 2026-04-17 **V2/V4 apply-only session** (follow-up session later the same day that consumed the ~5,700 proposals via 7 parallel file-owned subagents, landed 6 operator reattributions + typo rename + missing Charter Street + Ten Degrees HomeViews refresh + 4 age-cohort fixes + full V2 enrichment + full V4 area research + targeted Kew Bridge C→B V3 change — 59 files / +6965 / -634 in commit `e5506ed`, plus `d7a24d9` for the V3 grade change; full gate clean (tsc + validate-areas 0 errors 0 warnings + next build compiled)).
